...
MODULE_NAME_PATTERN = r"[A-Z][A-Za-z0-9_]*(\.[A-Z][A-Za-z0-9_]*)*"
MODULE_DECLARATION_RE = re.compile(r"^\s*module\s+(" + MODULE_NAME_PATTERN + r")\b", re.MULTILINE)
DEFAULT_SNIPPET_MODULE = "UserProgram"
...
class GraphSuiteHandler(SimpleHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        try:
            ...
            requested_module = str(payload.get("module", DEFAULT_SNIPPET_MODULE))
            vertices = parse_vertices(payload.get("vertices", list(range(6))))
            if not isinstance(source, str) or not source.strip():
                raise ValueError("source must be a non-empty string")
            source, module_name = prepare_source(source, requested_module)
        except Exception as exc:
            self.write_json(400, {"ok": False, "error": f"Bad request: {exc}"})
            return

        try:
            payload = compile_and_run(source, module_name, vertices)
        except Exception as exc:
            payload = {
                "ok": False,
...
def is_valid_module_name(module_name):
    return bool(re.fullmatch(MODULE_NAME_PATTERN, module_name))


def module_name_from_source(source):
    match = MODULE_DECLARATION_RE.search(source)
    if not match:
        raise ValueError(
            "source must include a named module declaration, "
            "such as: module Examples.Sandbox where"
        )

    module_name = match.group(1)
    if not is_valid_module_name(module_name):
        raise ValueError("source module name is not valid")
    if module_name == "Main":
        raise ValueError(
            "the public compiler runs exported graphs functions; "
            "use a named module instead of Main"
        )
    return module_name
...
def parse_vertices(value):
    if isinstance(value, int):
        if value < 0:
            raise ValueError("vertices must be a list of integers or integer pairs")
        return list(range(value))
    if not isinstance(value, list):
        raise ValueError("vertices must be a list of integers or integer pairs")
    if all(isinstance(vertex, int) for vertex in value):
        return value
    if all(
        isinstance(vertex, list)
        and len(vertex) == 2
        and all(isinstance(coordinate, int) for coordinate in vertex)
        for vertex in value
    ):
        return [tuple(vertex) for vertex in value]
    raise ValueError("vertices must be a list of integers or integer pairs")


def haskell_list_literal(values):
    return "[" + ",".join(haskell_value_literal(value) for value in values) + "]"


def haskell_value_literal(value):
    if isinstance(value, tuple):
        return "(" + ",".join(haskell_value_literal(part) for part in value) + ")"
    return str(value)


def prepare_source(source, requested_module):
    match = MODULE_DECLARATION_RE.search(source)
    if match:
        return source, module_name_from_source(source)

    return wrapped_snippet_source(source, DEFAULT_SNIPPET_MODULE), DEFAULT_SNIPPET_MODULE


def wrapped_snippet_source(source, module_name):
    return "\n".join(
        [
            ...
            "import Examples.EvenLength (evenLength)",
            "import Examples.Fibonacci (fibonacciTilings)",
            "import Examples.IntegerPartitions (integerPartitions)",
            "import Examples.MotzkinGraphs (motzkinGraphs)",
            "import Examples.OrderedBinaryTrees (orderedBinaryTrees)",
            "import Examples.RecursiveTotalOrders (recursiveTotalOrders)",
            "import Examples.BasicGraphs hiding (graphs, main)",
            "import Examples.SimpleGraphs",
            "  ( simpleGraphs",
            "  , connectedSimpleGraphs",
            "  , unlabelledSimpleGraphs",
            "  , unlabelledConnectedSimpleGraphs",
            "  )",
            "import Examples.UnorderedBinaryTrees (unorderedBinaryTrees)",
            "import Examples.UpToHidden (connectedUpTo)",
            "import GraphRenderer",
            "",
            source.strip(),
            "",
            "graphs vertices = runProgram output vertices",
            "",
        ]
    )
...
def runner_source(module_name, vertices):
    return "\n".join(
        [
            "module Main where",
            "",
            ...
            "",
            "main :: IO ()",
            "main = render (Program.graphs " + haskell_list_literal(vertices) + ")",
            "",
        ]
    )


def compile_and_run(source, module_name, vertices=None):
    if vertices is None:
    ...
    try:
        ...
        runner_path.write_text(runner_source(module_name, vertices), encoding="utf-8")
...
def validate_frames_text(text):
    ...
    for line_number, line in enumerate(text.splitlines(), start=1):
        ...
        if frame.get("kind") == "latticePath":
            if not isinstance(frame.get("p"), list):
                return f"Run produced a lattice path frame without a p list on line {line_number}.", frame_count
        elif not isinstance(frame.get("v"), list) or not isinstance(frame.get("e"), list):
