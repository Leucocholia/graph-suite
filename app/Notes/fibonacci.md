...
Species expression. The construction is `unionGraphs <|> tile`, where `tile` is the sum of an allowed singleton complete block and an allowed pair complete block.
...
Try this. Remove the size \(1\) tile or the size \(2\) tile from the `+` expression and watch the recurrence collapse.
