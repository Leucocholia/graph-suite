...
Species expression. The code uses `integerPartitionSpeciesWith unionGraphs indiscrete`. Instead of enumerating every labelled set partition, it enumerates the weakly decreasing list of block sizes and then uses the current label order to draw one representative.
