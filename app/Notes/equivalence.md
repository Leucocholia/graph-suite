...
Species expression. The code writes this as `unionGraphs <@> complete`. First partition the labels into nonempty blocks, then put one clique structure on each block, then union the block drawings.

Visual encoding. Each block is rendered as a directed clique with self-loops, so vertices are reflexive and two distinct vertices are mutually adjacent exactly when they lie in the same equivalence class.
